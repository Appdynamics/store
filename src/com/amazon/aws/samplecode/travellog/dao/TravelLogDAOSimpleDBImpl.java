/*
 * Copyright 2010-2012 Amazon.com, Inc. or its affiliates. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License").
 * You may not use this file except in compliance with the License.
 * A copy of the License is located at
 *
 *  http://aws.amazon.com/apache2.0
 *
 * or in the "license" file accompanying this file. This file is distributed
 * on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing
 * permissions and limitations under the License.
 */
package com.amazon.aws.samplecode.travellog.dao;

import java.util.*;

import javax.persistence.*;

import com.amazon.aws.samplecode.travellog.aws.S3PhotoUtil;
import com.amazon.aws.samplecode.travellog.aws.S3StorageManager;
import com.amazon.aws.samplecode.travellog.aws.TravelLogSNSManager;
import com.amazon.aws.samplecode.travellog.entity.*;
import com.amazon.aws.samplecode.travellog.util.StageUtils;

public class TravelLogDAOSimpleDBImpl implements TravelLogDAO {

    private static Map<String, String> properties = new HashMap<String, String>();
    static {   
        properties.put("lobBucketName", S3StorageManager.getKey().toLowerCase() + "-travellog-lob" + StageUtils.getResourceSuffixForCurrentStage());
    }

    public TravelLogDAOSimpleDBImpl() {

    }

    private static EntityManagerFactory factory = Persistence.createEntityManagerFactory("travellog");

    public Journal getJournal(String journalId) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            Journal journal = em.find(Journal.class, journalId);
            em.refresh(journal);
            return journal;
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }

    }

    public Journal saveJournal (Journal journal) {
        EntityManager em = null;
        //Storage fails if id is an empty string, so nullify it
        if (journal.getId()!=null && journal.getId().equals("")) {
            journal.setId(null);
        }
        try {
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            em.persist(journal);
            t.commit();

        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
        return journal;
    }

    public void deleteJournal(Journal journal) {

        //reload journal from DB so we get the entries
        //since cascades are not automatic
        EntityManager em = null;
        try {
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            journal = em.find(Journal.class,journal.getId());
            for (Entry entry:journal.getEntries()) {
                for(Photo photo:entry.getPhotos()) {
                    em.remove(photo);
                }

                for (Comment comment: entry.getComments()) {
                    em.remove(comment.getCommenter());
                    em.remove(comment);
                }
                em.remove(entry);
            }


            em.remove(journal);

            t.commit();

        } finally {
            if (em!=null) {
                em.close();
            }
        }
        TravelLogSNSManager snsManager = new TravelLogSNSManager();
        snsManager.cleanupTopics();
        
    }

    public void deleteJournals() {
        EntityManager em = null;
        try {
            em = factory.createEntityManager();
            Query query = em.createQuery("select j from com.amazon.aws.samplecode.travellog.entity.Journal j");
            List<Journal> journals =  query.getResultList();
            EntityTransaction t = em.getTransaction();
            t.begin();
            for (Journal journal:journals) {
                em.remove(journal);
            }

            t.commit();
        } finally {
            if (em!=null) {
                em.close();
            }
        }
    }


    @SuppressWarnings("unchecked")
    public List<Journal> getJournals() {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            Query query = em.createQuery("select j from com.amazon.aws.samplecode.travellog.entity.Journal j");
            return (List<Journal>)query.getResultList();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }

    public Journal updateJournal(Journal journal) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            journal = em.merge(journal);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
          return journal;
    }


    public User getUser(String username) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            Query query = em.createQuery("select u from com.amazon.aws.samplecode.travellog.entity.User u where u.username=:username");
            query.setParameter("username", username);
            return (User)query.getSingleResult();
        }
        catch (NoResultException e) {
            //No matching result so return null
            return null;
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }


    public void saveUser(User user) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();;
            em.persist(user);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }


    public void deleteUser(User user) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            user = em.find(User.class,user.getId());
            em.remove(user);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }


    public void deleteEntry(Entry entry)  {
        EntityManager em = null;
        List<Photo> photos = getPhotos(entry);

        try {
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            //Need to iterate through the associated photos
            //and remove them from S3
            for (Photo photo:photos) {
                S3PhotoUtil.deletePhotoFromS3(photo);
                em.remove(photo);
            }

            entry = em.find(Entry.class,entry.getId());
            em.remove(entry);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }

    }

    @SuppressWarnings("unchecked")
    public List<Entry> getEntries(Journal journal) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            journal = em.find(Journal.class,journal.getId());
            em.refresh(journal);
            return journal.getEntries();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }


    public Entry getEntry(String entryId) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            Entry entry = em.find(Entry.class, entryId);
            return entry;
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }


    public void saveEntry(Entry entry) {
        EntityManager em = null;
        try {
            //Storage fails if id is an empty string, so nullify it
            if (entry.getId()!=null && entry.getId().equals("")) {
                entry.setId(null);
            }

            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            em.persist(entry);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }

    public void refreshEntry(Entry entry) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            em.refresh(entry);
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }


    public void deletePhoto(Photo photo)  {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            em.remove(photo);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }

    }


    public void savePhoto(Photo photo) {
        EntityManager em = null;

        try {
            //Storage fails if id is an empty string, so nullify it
            if (photo.getId()!=null && photo.getId().equals("")) {
                photo.setId(null);
            }
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            em.persist(photo);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }

    }

    public void updatePhoto(Photo photo) {
        EntityManager em = null;

        try {
            //Storage fails if id is an empty string, so nullify it
            if (photo.getId()!=null && photo.getId().equals("")) {
                photo.setId(null);
            }
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            em.merge(photo);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }

    }

    @SuppressWarnings("unchecked")
    public List<Photo> getPhotos(Entry entry) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            Entry e  = em.find(Entry.class, entry.getId());
            return e.getPhotos();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }




    public Journal getJournal() {
        List<Journal> journals = getJournals();
        for (Journal journal: journals) {
            return journal;
        }
        return null;
    }


    public Photo getPhoto(String photoId) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            Photo photo = em.find(Photo.class, photoId);
            return photo;
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }

    @SuppressWarnings("unchecked")
    public List<User> getUsers() {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            Query query = em.createQuery("select u from com.amazon.aws.samplecode.travellog.entity.User u");
            return (List<User>)query.getResultList();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }




    public void deleteComment(Comment comment) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            comment = em.find(Comment.class,comment.getId());
            em.remove(comment.getCommenter());
            em.remove(comment);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }


    public void deleteCommenter(Commenter commenter) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            em.remove(commenter);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }

    @SuppressWarnings("unchecked")
    public List<Comment> getComments(Entry entry) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            entry = em.find(Entry.class, entry.getId());
            return entry.getComments();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }


    public void saveComment(Comment comment) {
        EntityManager em = null;
        if (comment.getId()!=null && comment.getId().equals("")) {
            comment.setId(null);
        }
        try {
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            em.persist(comment);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }


    public void saveCommenter(Commenter commenter) {

        EntityManager em = null;

        try {
            if (commenter.getId()!=null && commenter.getId().equals("")) {
                commenter.setId(null);
            }
            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            em.persist(commenter);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }


    public Comment getComment(String commentId) {
        EntityManager em = null;

        try {
            em = factory.createEntityManager();
            Comment comment = em.find(Comment.class, commentId);
            return comment;
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }

    public Entry updateEntry(Entry entry) {
        EntityManager em = null;
        Entry mergedEntry = entry;
        try {
            //Storage fails if id is an empty string, so nullify it
            if (entry.getId()!=null && entry.getId().equals("")) {
                entry.setId(null);
            }

            em = factory.createEntityManager();
            EntityTransaction t = em.getTransaction();
            t.begin();
            mergedEntry = em.merge(entry);
            t.commit();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
        return mergedEntry;
    }

    public List<Itinerary> getBookedIntinerary() {
        EntityManager em = null;
        Random r = new Random();
        int Low = 100;
        int High = 15000;
        int resultCount = r.nextInt(High-Low) + Low;
        try {
            em = factory.createEntityManager();
            Query q = em.createQuery("SELECT i FROM com.amazon.aws.samplecode.travellog.entity.Itinerary i");
            q.setMaxResults(resultCount);
            return q.getResultList();
        }
        finally {
            if (em!=null) {
                em.close();
            }
        }
    }
}
